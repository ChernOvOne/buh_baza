...
function migrate() {
  ...
  if (partnerCount === 0) {
    db.prepare(`INSERT INTO partners (name,role,share_type,share_value,color,sort_order) VALUES (?,?,?,?,?,?)`).run('Артём','owner','none',0,'#00e5ff',1);
    db.prepare(`INSERT INTO partners (name,role,share_type,share_value,color,sort_order) VALUES (?,?,?,?,?,?)`).run('Роман','investor','none',0,'#f97316',2);
    db.prepare(`INSERT INTO partners (name,role,share_type,share_value,color,sort_order) VALUES (?,?,?,?,?,?)`).run('Михаил','owner','none',0,'#00f5a0',3);
  }
  ...
  const migrated = db.prepare('SELECT COUNT(*) as c FROM partner_daily').get().c;
  if (migrated === 0) {
